...
class _global_context_layer(nn.Module):
    def __init__(self, c_in, c_out, c_mid=256, ks=15):
        ...
        self.row_prev = nn.Conv2d(c_in, c_mid , kernel_size=(ks,1))
        self.row_post = nn.Conv2d(c_mid,c_out , kernel_size=(1,ks))
        self.col_prev = nn.Conv2d(c_in, c_mid , kernel_size=(1,ks))
        self.col_post = nn.Conv2d(c_mid,c_out , kernel_size=(ks,1))
        self._init_weights()
...
# rfcn of light head version.
class _rfcn_lighthead(nn.Module):
    """ faster RCNN """
    def __init__(self, classes, class_agnostic, mid_c = 10):
        super(_rfcn_lighthead, self).__init__()
        self.classes = classes
        self.mid_c = mid_c # mid_c is the output dim of psroi pooling.
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        # before using the pooled feat, we apply a linear layer.
    ...
    def _init_weights(self):
        ...
        normal_init(self.fc_roi, 0, 0.005, cfg.TRAIN.TRUNCATED)
